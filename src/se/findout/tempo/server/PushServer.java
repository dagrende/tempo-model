...
public class PushServer {
	...
	/**
	 * Sends a message on an identified channel.
	 * 
	 * @param playerKeys
	...
	public static void sendMessageByKey(String channelId, ChangeInfo msg) {
		...
		try {
			...
			// TODO A bug in the dev_appserver causes an exception to be thrown
			// when no users are connected yet.
			// Rethink this when that bug is taken care of.
			logger.log(Level.SEVERE, "Failed to push the message " + msg + " to client "
					+ channelId, e);
		}
	}
	...
	private static SerializationPolicy createPushSerializationPolicy() {
		...
		File[] files = new File("tempo").listFiles(new FilenameFilter() {
